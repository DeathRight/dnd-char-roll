...
export function genUntilPass(callback, i) {
  let n = callback(i);
  while (n === "") {
    n = callback(i);
  }
 ...
 * Type: 1 = female, 3 = old male, 3 = old female, default: male
 */
export function nameGen(type) {
  const i = Math.floor(Math.random() * 17);
  let name = "";
  let lname = genUntilPass(nameSur, i);

  switch (type) {
    case 1:
      name = genUntilPass(nameFem, i);
      break;
    case 3:
      name = nm83[Math.floor(Math.random() * nm83.length)];
      break;
    case 4:
      name = nm84[Math.floor(Math.random() * nm84.length)];
      break;
    default:
      name = genUntilPass(nameMas, i);
      break;
  }
  return `${name} ${lname}`;
}

export function nameSur(i) {
  let lname;
  let rnd;
  let rnd2;
  let rnd3;
